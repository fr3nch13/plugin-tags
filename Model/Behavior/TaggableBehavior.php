 ...
 */
	protected $_defaults = array(
		...
		'resetBinding' => false,
		'update_tags_field' => 'update_tags',
		'taggedCounter' => false,);
	
	private $_taggedModel = false;
 ...
 */
	...
	public function beforeDelete(Model $Model, $cascade = true)
	{
		...
		if(in_array($Model->alias, array('Tag', 'Tagged')))
		{
		...
		// if many are being deleted, this would have been removed
		// this also basically will catch if 
		if(!$Model->{$taggedAlias} and !$this->_taggedModel)
		{
			return parent::beforeDelete($Model, $cascade);
		}
		
		if(isset($Model->hasAndBelongsToMany[$tagAlias]))
			unset($Model->hasAndBelongsToMany[$tagAlias]);
		
		// unbind the tagged, and tag models, from $Model to make sure the core deleting doesn't get ran on them
		if($Model->{$taggedAlias})
		{
			$Model->unbindModel(
				array('hasAndBelongsToMany' => array($tagAlias))
			);
			
			// store a copy of the Tagged model within this instance
			$this->_taggedModel = $Model->{$taggedAlias};
			
			// destroy the tag, and tagged model instances
			$Model->{$tagAlias} = false;
			$Model->{$taggedAlias} = false;
		}
		
		// delete any tagged records tagged to the object, before any records are deleted by cakephp's core Model class
		$conditions = array(
			$taggedAlias.'.foreign_key' => $Model->id,
			$taggedAlias.'.model' => $Model->name,
		);
		
		// delete all of this object's tadded records
		$this->_taggedModel->deleteAll($conditions);
 ...
 */
	public function disassembleTags(Model $Model, $string = '', $separator = ',') {
		...
		foreach ($array as $tag) {
			...
			$tag = trim($tag);
			if (!empty($tag)) {
				$key = $this->multibyteKey($Model, $keyname);
				if (empty($tags[$key])) {
			...
			if (!empty($tags)) {
				...
				if (!empty($existingTags)) {
					foreach ($existingTags as $existing) {
						$_keyname = $existing[$tagAlias]['keyname'];
						$existingTagKeyNames[$_keyname] = $existing[$tagAlias]['keyname'];
						$existingTagIds[$_keyname] = $existing[$tagAlias]['id'];
						$existingTagIdentifiers[$_keyname] = $existing[$tagAlias]['identifier'];
					}
				...
				foreach ($newTags as $key => $newTag) {
					$newTag = array($tagModel->alias => $newTag);
					$tagModel->create();
					...
					if ($update == true) {
						$oldTagIds = $tagModel->Tagged->find('all', array(
							...
							'conditions' => array(
								...
								'Tagged.language' => Configure::read('Config.language')),
							'fields' => 'Tagged.tag_id'));
						if(!is_array($oldTagIds)) $oldTagIds = array();
					...
					if ($this->settings[$Model->alias]['cacheOccurrence']) {
						$newTagIds = $tagModel->Tagged->find('all', array(
							...
							'conditions' => array(
								...
								'Tagged.language' => Configure::read('Config.language')),
							'fields' => 'Tagged.tag_id'));
						if(!is_array($newTagIds)) $newTagIds = array();
...
/**
 * Allows you to transfer all tags from one associated object, to another object
 *
 * @param AppModel $Model
 */
	public function transferTagged(Model $Model, $oldForeignKey = false, $oldModel = false, $newForeignKey = false, $newModel = false)
	{
		return true;
		if(!$oldForeignKey or !$oldModel or !$newForeignKey or !$newModel)
		{
			$Model->modelError = __('Missing paramaters');
			return false;
		}
		$tagAlias = $this->settings[$Model->alias]['tagAlias'];
		$taggedAlias = $this->settings[$Model->alias]['taggedAlias'];
		
		return $Model->{$tagAlias}->{$taggedAlias}->updateAll(
			array($taggedAlias.'.foreign_key' => $newForeignKey, $taggedAlias.'.model' => "'".$newModel."'"),
			array($taggedAlias.'.foreign_key' => $oldForeignKey, $taggedAlias.'.model' => $oldModel)
		);
	}
 ...
 */
	...
	{
		if ($this->settings[$Model->alias]['automaticTagging'] == true && !empty($Model->data[$Model->alias][$this->settings[$Model->alias]['field']]) && $Model->id) 
		{
			// append the tags_type to the tags
			$type_field = false;
			if(isset($Model->data[$Model->alias][$this->settings[$Model->alias]['type_field']]))
			{
				$type_field = $Model->data[$Model->alias][$this->settings[$Model->alias]['type_field']];
			}
			if($type_field)
			{
				$type_field = $Model->alias. ' Type: '. $type_field;
				$Model->data[$Model->alias][$this->settings[$Model->alias]['field']] .= ', '. $type_field;
			}
			
			$update = true;
			if(isset($Model->data[$Model->alias][$this->settings[$Model->alias]['update_tags_field']][$this->settings[$Model->alias]['field']]))
			{
				
				if(!$Model->data[$Model->alias][$this->settings[$Model->alias]['update_tags_field']][$this->settings[$Model->alias]['field']])
				{
					$update = false;
				}
			}
			
			$this->saveTags($Model, $Model->data[$Model->alias][$this->settings[$Model->alias]['field']], $Model->id, $update);
		}
	}
	...
	public function beforeSave(Model $Model, $options = array())
	{
		if(in_array($Model->alias, array('Tag', 'Tagged')))
		{
			if ($Model->Behaviors->enabled('Tags.Taggable'))
			{
				$Model->Behaviors->disable('Tags.Taggable');
			}
		}
	}
 ...
 */
	public function afterFind(Model $Model, $results = array(), $primary = false) 
	{
		...
		$identifier = $Model->alias. ' Type:';
		
		foreach ($results as $key => $row) 
		{
			if(!isset($row[$Model->alias])) continue; 
			
			$row[$Model->alias][$field] = '';
			$row[$Model->alias][$type_field] = '';
			
			if (isset($row[$tagAlias]) && !empty($row[$tagAlias])) 
			{
				$row[$Model->alias][$field] = $this->tagArrayToString($Model, $row[$tagAlias]);
				
				if($row[$Model->alias][$field])
				{
					// look for the tags type in the tags
					$tags = explode($separator, $row[$Model->alias][$field]);
					foreach($tags as $i => $tag)
					{
						$tag = trim($tag);
						if(stripos($tag, $identifier) !== false)
						{
							$row[$Model->alias][$type_field] = trim(array_pop(explode(':', $tag)));
							unset($tags[$i]);
							break;
						}
					}
					$row[$Model->alias][$field] = implode($separator. ' ', $tags);
				}

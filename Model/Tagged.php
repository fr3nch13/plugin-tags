 ...
 */
	...
	public $validate = array(
		'foreign_key' => array('rule' => 'notBlank'),
		'tag_id' => array('rule' => 'notBlank'),
		'model' => array('rule' => 'notBlank'),
	);
 ...
 */
		...
		if ($state == 'before') {
			...
			if (isset($query['model']) && $Model = ClassRegistry::init($query['model'])) {
				...
				if ($query['recursive'] == -1) {
					throw new InvalidArgumentException(__d('tags', 'Find tagged will not work with a recursive level of -1, you need at least 0.'), E_USER_ERROR);
				}
	...
	function taggedSql($key_name = false, $model = false)
	{
		...
		// get the model ids tagged by this tag
		...
		$subQuery = $db->buildStatement(
			array(
				'fields'	 => array('`'. $this->alias. '`.`foreign_key`'),
